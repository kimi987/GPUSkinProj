...
namespace AnimationInstancing
{
    ...
    public class AnimationInstancingMgr : AnimationInstancingSingleton<AnimationInstancingMgr>
    {
        ...
        public static string folderName = "AnimationInstancingAnimationTextureData";
        public static string AnimationTextureDataPath ="/DataExport/" + folderName + "/";
        
        private Dictionary<int, bool> instancePool;
        ...
        public void Init()
        {
            ...
            aniInstancingList = new List<AnimationInstancing>(50000);
            ...
            instancePool = new Dictionary<int, bool>();
        }
...
//                 vertexCache.currentInstancingIndex = 0;
        ...
        void ApplyBoneMatrix()
        {
            ...
            {
                ProcessInstance(aniInstancingList[i], i);
            }
        }
        void ProcessInstance(AnimationInstancing instance, int index)
        {
            ...
            boundingSphere[index] = instance.boundingSpere;

            if (!instance.visible)
                return;
            instance.UpdateLod(cameraTransform.position);

            ProcessMaterialBlockAndInstanceData(instance);
        }

        void ProcessMaterialBlockAndInstanceData(AnimationInstancing instance)
        {
            AnimationInstancing.LodInfo lod = instance.lodInfo[instance.lodLevel];
            int aniTextureIndex = instance.parentInstance != null ? instance.parentInstance.aniTextureIndex : instance.aniTextureIndex;
            ...
            for (int j = 0; j != lod.vertexCacheList.Length; ++j)
            {
                ...
                if (package.instancingCount + 1 > instancingPackageSize)
                {
                    ...
                    {
                        InstancingPackage newPackage = CreatePackage(block.instanceData, cache.mesh, cache.materials, aniTextureIndex);
                ...
                UpdateInstanceData(instance, block, aniTextureIndex, packageIndex);
            }
        }

        void UpdateInstanceData(AnimationInstancing instance, MaterialBlock block, int aniTextureIndex, int packageIndex)
        {
            // 获取实例数据和相关索引
            InstanceData data = block.instanceData;
            int runtimePackageIndex = block.runtimePackageIndex[aniTextureIndex];
            InstancingPackage pkg = block.packageList[aniTextureIndex][runtimePackageIndex];
            int count = pkg.instancingCount - 1;

            if (count < 0) return; // 如果没有有效的实例数量，直接退出

            // 获取世界矩阵
            Matrix4x4 worldMat = instance.worldTransform.localToWorldMatrix;

            // 更新世界矩阵数组
            Matrix4x4[] arrayMat = data.worldMatrix[aniTextureIndex][runtimePackageIndex];
            arrayMat[count] = worldMat;

            // 计算帧索引和过渡进度
            float frameIndex, preFrameIndex = -1, transition = 0f;
            AnimationInfo currentAniInfo, preAniInfo;

            if (instance.parentInstance != null)
            {
                currentAniInfo = instance.parentInstance.aniInfo[instance.parentInstance.aniIndex];
                preAniInfo = instance.parentInstance.preAniIndex >= 0 
                    ? instance.parentInstance.aniInfo[instance.parentInstance.preAniIndex] 
                    : null;

                frameIndex = currentAniInfo.animationIndex + instance.parentInstance.curFrame;
                if (preAniInfo != null)
                    preFrameIndex = preAniInfo.animationIndex + instance.parentInstance.preAniFrame;
                transition = instance.parentInstance.transitionProgress;
            }
            else
            {
                currentAniInfo = instance.aniInfo[instance.aniIndex];
                preAniInfo = instance.preAniIndex >= 0 
                    ? instance.aniInfo[instance.preAniIndex] 
                    : null;

                frameIndex = currentAniInfo.animationIndex + instance.curFrame;
                if (preAniInfo != null)
                    preFrameIndex = preAniInfo.animationIndex + instance.preAniFrame;
                transition = instance.transitionProgress;
            }

            // 更新帧索引和过渡进度
            data.frameIndex[aniTextureIndex][runtimePackageIndex][count] = frameIndex;
            data.preFrameIndex[aniTextureIndex][runtimePackageIndex][count] = preFrameIndex;
            data.transitionProgress[aniTextureIndex][runtimePackageIndex][count] = transition;
        }
        ...
        public void SetupAdditionalData(VertexCache vertexCache)
        {
            ...
            List<Vector4> uv1 = new List<Vector4>(vertexCache.boneIndex.Length);
            for (int i = 0; i != vertexCache.boneIndex.Length; ++i)
            {
                uv1.Add(vertexCache.boneIndex[i]);
            }
            vertexCache.mesh.SetUVs(1, uv1);
        ...
        public bool IsInInstancePool(string instanceName)
        {
            if (!instancePool.ContainsKey(instanceName.GetHashCode()))
            {
                instancePool.Add(instanceName.GetHashCode(),true);
                return false;
            }
            return true;
        }
