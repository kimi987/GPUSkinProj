...
namespace AnimationInstancing
{
    ...
    public class AnimationInstancing : MonoBehaviour
    {
        ...
        [HideInInspector]
        public string initAniName = "idle";
        ...
        private static string DefaultAnimationName = "idle";  //没有动作时播idle
        ...
        public void Init()
        {
            ...
            DestroyChildrenIfInInstancePool();
            IsInit = true;
        }
        private void DestroyChildrenIfInInstancePool()
        {
            if (AnimationInstancingMgr.Instance.IsInInstancePool(transform.name))
            {
        ...
        public bool InitializeAnimation()
        {
            ...
            AnimationManager.InstanceAnimationInfo info = AnimationManager.Instance.FindAnimationInfoByAssetManager(prototype);
            ...
            PlayAnimation(initAniName ?? aniInfo[0].animationName);
            return true;
        }
        ...
        public bool PlayAnimation(int animationIndex, float startFrame = 0f)
        {
            startFrame -= 1;//帧数从0开始
            if (aniInfo == null)
            {
            ...
            // 如果请求播放同一动画且未暂停，且起始帧与当前帧相同，则返回false
            if (animationIndex == aniIndex && !IsPause() && Mathf.Approximately(startFrame, curFrame))
            {
                return false;
            }
            ...
            {
                AnimationInfo animation = aniInfo[animationIndex];
                int totalFrames = animation.totalFrame;
                ...
                preAniFrame = (int)(curFrame + 0.5f);
                curFrame = startFrame;
                eventIndex = -1;
                preAniTextureIndex = aniTextureIndex;
                aniTextureIndex = animation.textureIndex;
        ...
        public bool CrossFade(string animationName, float duration, float startFrame = 0f)
        {
            int hash = animationName.GetHashCode();
            int index = FindAnimationInfo(hash);
            if (index < 0)
            {
                index = FindAnimationInfo(DefaultAnimationName.GetHashCode());
                CrossFade(index, duration, startFrame);
                return false;
            }
            return CrossFade(index, duration, startFrame);
        }

        private bool CrossFade(int animationIndex, float duration, float startFrame = 0f)
        {
            var isSuccessPlay = PlayAnimation(animationIndex, startFrame);
            if (duration > 0.0f)
            {
                isInTransition = true;
   ...
   //      }
        ...
        public void JumpFrame(int frameCount)
        {
            if (frameCount <= 0)
            {
                return;
