...
namespace AnimationInstancing
{
    public class AnimationGenerator : EditorWindow
    {
        ...
        public void MakeWindow(GameObject prefab,bool generateAndClose = false)
        {
            s_window = GetWindow(typeof(AnimationGenerator)) as AnimationGenerator;
            generatedPrefab = prefab;
            if (generateAndClose)
            {
                EditorApplication.delayCall += BakeWithAnimatorAndClose;
            }
        }

        void BakeWithAnimatorAndClose()
        {
            Debug.LogError("BakeWithAnimatorAndClose");
            GetClips(generatedPrefab.GetComponentInChildren<Animator>());
            ...
            do
            {
                ...
                if (workingInfo != null)
                {
                    go = workingInfo.animator.gameObject;
                }
        ...
        {Debug.LogError("BakeWithAnimator");
            if (generatedPrefab != null)
            {
        ...
        void AnalyzeStateMachine(UnityEditor.Animations.AnimatorStateMachine stateMachine,
            ...
            int animationIndex)
        {
            Debug.LogError("AnalyzeStateMachine");
        ...
        {
            string path = 
                Application.dataPath + AnimationInstancingMgr.AnimationTextureDataPath;
            if (!Directory.Exists(path))
                AssetDatabase.CreateFolder("Assets", AnimationInstancingMgr.folderName);
            ...
            Debug.Log($"生成AnimationInfo成功，路径：{path + name + ".bytes"}");
        }
            ...
            bool rootMotion)
        {
            ...
            Debug.LogError("GenerateBoneMatrix");
        ...
        private void AddMeshVertex2Generate(SkinnedMeshRenderer[] meshRender,
            ...
            Matrix4x4[] bindPose)
        {
            Debug.LogError("AddMeshVertex2Generate");
